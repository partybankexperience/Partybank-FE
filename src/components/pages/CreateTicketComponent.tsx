...
const CreateTicketComponent = () => {
  ...
  const isManageEventsContext =
    location.pathname.includes('/manageEvents') && location.pathname.includes('/createTicket');
  ...
  // Event ID Source
  const eventId = isCreateEventContext ? localStorage.getItem('eventId') : location.state?.id;
  ...
  useEffect(() => {
    (window as any).focusFirstTicketError = () => {
      ...
      else if (manageTicketErrors.category && ticketCategoryRef.current)
        ticketCategoryRef.current.focus();
      else if (manageTicketErrors.price && priceRef.current) priceRef.current.focus();
      else if (manageTicketErrors.salesStart && salesStartRef.current)
  ...
  const handleManageEventsSubmit = async () => {
    ...
    try {
      ...
      if (ticketValue && ticketValue !== undefined) {
        ...
        await editTicket(
          ...
          Array.isArray(ticketData.perks)
            ? ticketData.perks.filter((perk) => perk && perk.trim())
            : [],
      ...
      markTicketAsSaved(
        manageActiveIndex,
        response.id || response.ticketId || currentTicket?.savedTicketId,
      );
    ...
    <div className="grid gap-[2.5rem] mt-[20px]">
      <div className="grid gap-[15px]">
        ...
        {errorField.ticketCategory && (
          <p className="text-[13px] text-red-500 mt-1">{errorField.ticketCategory}</p>
        )}
      </div>
      ...
      <div className="grid md:flex justify-between gap-[15px]">
        <div className="grid gap-[15px]">
          ...
          <div className="grid md:flex gap-4 md:gap-[2.5rem]" ref={ticketTypeRef}>
            {ticketType.map((type) => (
              <RadioButton
                key={type}
                label={type}
          ...
          {errorField.ticketType && (
            <p className="text-[13px] text-red-500 mt-1">{errorField.ticketType}</p>
          )}
        </div>

        <div className="grid gap-[15px]">
          <h2 className="text-black text-[1.2rem] font-bold">Ticket Availability</h2>
          <div className="grid md:flex gap-4 md:gap-[2.5rem]" ref={ticketAvailabilityRef}>
          ...
          {errorField.isUnlimited && (
            <p className="text-[13px] text-red-500 mt-1">{errorField.isUnlimited}</p>
          )}
        </div>
      </div>
        ...
        <div className="grid gap-[15px] md:grid-cols-2">
          ...
          {getValue('ticketType') === 'Paid' && (
            <div className="grid gap-1 w-full relative">
              ...
              {errorField.price && (
                <p className="text-[13px] text-red-500 mt-1">{errorField.price}</p>
              )}
            </div>
          )}
          <DefaultInput
