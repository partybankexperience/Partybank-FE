...
const CreateTicketComponent = () => {
  ...
  const getValue = (key: string, value?: any) => {
    let retValue = "";
    if (isCreateEventContext) {
      retValue = form[currentStage]?.[key] || "";
    } else if (isManageEventsContext) {
      retValue = getCurrentTicketData(key) || "";
    }
    
    // Convert ISO dates to date input format for date fields
    if ((key === 'salesStart' || key === 'salesEnd') && retValue && retValue.includes('T')) {
      return convertISOToDateInput(retValue);
    }
    
    // Map backend field names to frontend field names
    if (key === 'ticketName') {
      retValue = getCurrentTicketData('name') || retValue;
    }
    
    if (key === 'totalStock') {
      retValue = getCurrentTicketData('totalStock') || retValue;
    }
    
    if (key === 'groupSize') {
      retValue = getCurrentTicketData('groupSize') || retValue;
    }
    
    if (key === 'ticketCategory') {
      const category = getCurrentTicketData('category') || retValue;
    ...
    if (key === 'ticketType') {
      const type = getCurrentTicketData('type') || retValue;
    ...
    if (key === 'color') {
      retValue = getCurrentTicketData('color') || '#ff0000';
    }
    return retValue;
  };
  ...
  const validateForm = () => {
    ...
    if (Object.keys(ticketErrors).length > 0) {
      setTimeout(() => {
        ...
        } else if (ticketErrors.color && colorRef.current) {
...
const CreateTicketComponent = () => {
  ...
  const addPerk = () => {
    const perks = getValue("perks");
    const perksArray = Array.isArray(perks) && perks.length > 0 ? perks : [""];
    handleInputChange("perks", [...perksArray, ""]);
  };
