...
const TicketSidebar = ({ onAddTicket, onEditTicket, onDeleteTicket }: TicketSidebarProps) => {
  ...
  const isManageEventsContext =
    location.pathname.includes('/manageEvents') && location.pathname.includes('/create-ticket');
  ...
  const activeTicketIndex = isCreateEventContext
    ? currentTicket?.activeTicketIndex || 0
    : manageActiveIndex;
        ...
        {allTicketForms.length > 0 && (
          <div className="mt-4 pt-3 border-t border-gray-200">
            <p className="text-xs text-gray-500 text-center">
              {allTicketForms.length} ticket{allTicketForms.length !== 1 ? 's' : ''} • Active:{' '}
              {activeTicketIndex + 1}
            </p>
          </div>
        )}
